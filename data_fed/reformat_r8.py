...
class REFORMAT_R8:
    ...
    def reformat_r8(self):
        file_paths_list = []
        content_list = []
        with open(os.path.join(path_r8, 'reuters_r8', 'R8_label.txt')) as f:
            for line in f.readlines():
                file_paths_list.append(line)
        with open(os.path.join(path_r8, 'reuters_r8', 'R8.txt')) as f:
            for line in f.readlines():
        ...
        for file_path in file_paths_list:
            primitive_topic_list.append(file_path.split('\t')[2])
        ...
        for i, file_path in enumerate(file_paths_list):
            number_topic_list.append(str(topic_dic[file_path.split('\t')[2]]))
        ...
        if len(file_paths_list) == len(content_list):
            pass
        else:
            exit('reformat_r8.py: len(file_paths_list) == len(content_list) 不相等')

        for file_path, number_topic, content in zip(file_paths_list, number_topic_list, content_list):
            if file_path.split('\t')[1] == 'test':
                self.test_all_content_list.append((file_path, number_topic, content))
            elif file_path.split('\t')[1] == 'train':
                self.train_all_content_list.append((file_path, number_topic, content))
            else:
                exit('reformat_r8.py: 存在 for file_path, number_topic, content in zip(file_paths_list, number_topic_list, content_list): 错误')
        print('test_all_content_list 样本如下：')
        print(self.test_all_content_list[0])
        print(self.test_all_content_list[1])
        print(self.test_all_content_list[2])
        print('train_all_content_list 样本如下：')
        random.shuffle(self.train_all_content_list)  # 对训练数据进行打乱
        print(self.train_all_content_list[0])
        print(self.train_all_content_list[1])
        print(self.train_all_content_list[2])


    def partition_r8(self):
        ...
        # 将总数据分给 num_nodes 个节点（每个 node 表示一个 user），有 num_users 个用户参与，每个 user 有 self.args.num_max_nums 数据参与
        num_items_user = int(len(partitioned_train_all_content_list_users) / self.args.max_nums)  # 每个用户的数据最多分为 self.args.max_nums 份
        ...
        print(f'总训练数据数：{len(self.test_all_content_list)}')
        ...
        with open(os.path.join(path_r8, 'r8.txt'), 'w') as f:
            for line in self.textgcn_path_list:
                f.write(str(line))  # line 有 \n
        with open(os.path.join(path_r8, 'corpus', 'r8.txt'), 'w') as f:
            for line in self.textgcn_content_list:
                f.write(str(line))  # line 有 \n

        with open(os.path.join(path_r8, 'r8.txt'), 'r') as f:
            print(f'os.path.join(path, "r8.txt"): {len(f.readlines())}')
        with open(os.path.join(path_r8, 'corpus', 'r8.txt'), 'r') as f:
            print(f'os.path.join(path, "corpus", "20ng.txt"): {len(f.readlines())}')
        self.copy_file(os.path.join(path_r8, 'r8.txt'), os.path.join(path_fed_avg, 'text_gcn/data/r8.txt'))
        self.copy_file(os.path.join(path_r8, 'corpus', 'r8.txt'), os.path.join(path_fed_avg, 'text_gcn/data/corpus/r8.txt'))

        # 测试 'text_gcn/data/r8.txt'
        print('测试 text_gcn/data/r8.txt')
        with open(os.path.join(path_fed_avg, 'text_gcn/data/r8.txt'), 'r') as f:
            print(f'os.path.join(path, "r8.txt"): {len(f.readlines())}')
        with open(os.path.join(path_fed_avg, 'text_gcn/data/corpus/r8.txt'), 'r') as f:
            print(f'os.path.join(path, "corpus", "r8.txt"): {len(f.readlines())}')
...
if __name__ == '__main__':
    ...
    dataset = REFORMAT_R8(args=args)
    dataset.reformat_r8()
    dataset.partition_r8()
